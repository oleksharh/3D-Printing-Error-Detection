...
def extract_scalar_events(path):
    ...
    train_acc = [e.value for e in ea.Scalars("train_acc")]
    step = len(train_acc) // 20
    train_acc = [train_acc[0]] + [train_acc[i * step] for i in range(1, 19)] + [train_acc[-1]]
...
plt.title("VM-UNet Stage1: Initial Layer Dataset", fontsize=14)
...
plt.tight_layout()
plt.savefig("report_metrics/vm_unet_stage1_accuracy_plot.svg", format="svg", bbox_inches="tight")
plt.show()
